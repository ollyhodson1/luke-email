...
function renderFolders() {
  ...
  document.getElementById("folders").innerHTML = folders.map((folder) => `
    <button class="folder-btn ${state.activeFolder === folder ? "active" : ""}" data-folder="${escapeHtml(folder)}">
      ...
      ${counts[folder] ? `<span class="count">${counts[folder]}</span>` : ""}
    </button>
  `).join("");
      ...
      <button class="message-card ${current?.id === email.id && !state.showCompose ? "active" : ""}" data-email="${escapeHtml(email.id)}">
        <div class="msg-top">
          <div style="min-width:0;">
            <div class="msg-subject">${escapeHtml(email.subject)}</div>
    ...
    <article class="panel">
      <div class="panel-head">
        <div style="min-width:0;">
          ...
          <div class="panel-sub">${escapeHtml(email.dateGroup)} · ${escapeHtml(email.time)}</div>
        </div>
...
function sendMail() {
  ...
  state.activeFolder = "Sent Items";
  state.selectedId = sent.id;
  state.showCompose = false;
  state.compose = { to: [], subject: "", body: "" };
  showToast("Email sent. Wait for a reply.", 3200);
  render();
  ...
  state.replyTimer = setTimeout(() => {
    ...
    state.selectedId = reply.id;
    showToast("New reply received from Becca Richardson.", 4200);
    render();
  }, 20000);
}
...
function updateClock() {
  document.getElementById("clock").innerHTML = `${nowLabel()}<br>${todayLabel()}`;
}
