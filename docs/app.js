...
function sendMail() {
  ...
  showToast("Email sent. The clock will move forward 20 minutes.", 2500);
  render();

  window.clearTimeout(state.replyTimer);
  state.replyTimer = window.setTimeout(() => {
    finalFastForwardClock(20, () => {
...
console.assert(typeof finalFastForwardClock === "function", "FINAL_CLOCK_20_BUILD: clock fast-forward is present.");
console.assert(window.INITIAL_EMAILS.some((email) => email.id === "continuity-becca-luke-contacted"), "FINAL_CLOCK_20_BUILD: continuity Becca email is present.");
